document.addEventListener("DOMContentLoaded", function() {
  ...
  function type() {
      ...
      if (isDeleting) {
          typeSpeed = 50; // Velocidade do apagamento
      } else {
      ...
      if (isDeleting) {
          ...
          currentPhrase.push(fullText[letterIndex]);
          letterIndex++;
      }

      // Adicionando o cursor ao final da frase atual
      typewriterElement.innerHTML = currentPhrase.join('') + `<span class="blink">|</span>`;

      if (!isDeleting && letterIndex === fullText.length) {
          typeSpeed = 1000; // Tempo em que a frase completa fica visível antes de começar a apagar
          isDeleting = true;
      } else if (isDeleting && letterIndex === 0) {
          isDeleting = false;
          currentPhraseIndex = (currentPhraseIndex + 1) % phrases.length;
          typeSpeed = 500; // Tempo antes de começar a próxima frase
      }
